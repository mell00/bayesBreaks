...
frequentist Bai–Perron style grid search, and a Bayesian breakpoint sampler.
...
clearly separated mean shifts. The raw line plot makes it obvious how long each
regime persists and how dramatic the shift is before any modelling.
...
A simple plot shows the level shifts built into the generator. The dashed red
lines mark the true changepoints baked into the helper so you can immediately
see where the algorithm should place breakpoints.
...
The `bai_perron_ar()` helper evaluates all possible segmentations up to a
...
The BIC curve highlights that the two-break model is preferred for this series. Look for the dashed vertical line: it marks the lowest BIC across models with a different number of breaks, making it easy to cross-check against the true changepoints drawn earlier.
...
individual break locations. The count bar chart shows where the MCMC spent most
of its time (higher bars), so narrow modes indicate a confident posterior on the
model size.
...
Even with a short run, the BAAR sampler concentrates near the two true
...
The histogram overlays show how often each time index appeared as a breakpoint
across MCMC draws. Dashed lines summarize the Bai-Perron estimate and the true
changepoints so you can see when the posterior favours a nearby position rather
than an exact match.

## Peeking at the C++ backend

The high-level R functions are backed by C++ routines in `src/` for speed. For
example, `rcpp_baar.cpp` contains the log-likelihood used inside the
Metropolis-Hastings steps. You can experiment with a lightweight variant of the
mean-shift log-likelihood directly from the vignette using `Rcpp::sourceCpp`.

```{r cpp-loglik, message=FALSE, eval = requireNamespace("RcppArmadillo", quietly = TRUE)}
Rcpp::sourceCpp(code = '
  #include <RcppArmadillo.h>
  #include <limits>
  // [[Rcpp::depends(RcppArmadillo)]]
  // [[Rcpp::export]]
  double segment_loglik_cpp(Rcpp::NumericVector x) {
    arma::vec segment = Rcpp::as<arma::vec>(x);
    double mean = arma::mean(segment);
    arma::vec centered = segment - mean;
    double sigma2 = arma::mean(arma::square(centered));
    if (!R_finite(sigma2) || sigma2 <= 0) {
      sigma2 = std::numeric_limits<double>::epsilon();
    }
    return -0.5 * static_cast<double>(segment.n_elem) *
      (std::log(2 * M_PI) + std::log(sigma2) + 1.0);
  }
')

segment_loglik_cpp(series$data_2)
```

The example shows the same structure as the internal C++ functions: convert the
R vector to an Armadillo object, compute summary statistics, guard against
numerical issues, and return a log-likelihood scalar. You can adapt this
pattern to prototype alternative priors or likelihoods without touching the R
wrapper code.
...
baseline and sampler still recover the main structure. Each plot in this section
calls out the placed changepoints so you can visually compare the truth to the
estimated quantities shown next.
...
breakpoints when the autoregressive order stays low. Use the dashed line in the
plot to spot the minimum BIC; if it hugs the true changepoints, the search is
providing a stable baseline before running the sampler.
...
locations despite the noise. The histogram and vertical overlays reveal when the
posterior allows slight jitter around the truth versus locking tightly onto the
known changepoints.
...
autoregressive order helps both the Bai-Perron search and BAAR sampler. We start with a
visual inspection to spot persistence changes, then align the autoregressive
order in the model to capture the regime shift.
